...
def biasedcoinflip(p=0.5):
    randomnumber = random.random()       # generate random number [0.0, 1.0)

    if randomnumber < p:               # makes the "coin" biased
        return 1                    # heads
    else:
        return 0                    # tails
...
for TrialIndex2 in range(0, NumberTrials):
    ...
    for Flip in range(0, NumberFlips):
        flipsSumNum = flipsSumNum + biasedcoinflip(ParameterP)
                                    # add NumberFlips coin flips for each SumTrials outcome
    SumTrials.append(flipsSumNum)
